...
def test_text_service_lifecycle() -> None:
    env = resolve_env()
    ...
    balance_before = get_credit_balance(api_url, access_token)
    proxy_url = f"{api_url}/services/text/{stub}?projectId={project_id}"
    proxy_payload = {"messages": [{"role": "user", "content": "Hello from lifecycle test."}]}
...
@pytest.mark.integration
def test_services_all_together() -> None:
    env = resolve_env()
    supabase_url = env.get("SUPABASE_URL", "").rstrip("/")
    service_key = env.get("SUPABASE_SERVICE_ROLE_KEY")
    if not supabase_url or not service_key:
    ...
    for service_type, entries in services.items():
        ...
        for service in entries:
            ...
            assert stub, f"missing stub for {service_type} service"
            enable_resp = requests.post(
                f"{api_url}/projects/{project_id}/services",
                json={"serviceStub": stub, "config": {}},
                headers=service_headers,
            ...
            project_service_key = create_project_service_key(api_url, service_key, project_id, stub)
            project_service_headers = {"x-project-service-key": project_service_key, "Content-Type": "application/json"}
            proxy_url = f"{api_url}/services/{service_type}/{stub}?projectId={project_id}"
            payload = {"messages": [{"role": "user", "content": "Hello from all-services test."}]} if service_type == "text" else {}
            proxy_resp = requests.post(
                proxy_url,
