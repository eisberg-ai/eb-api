...
def assert_non_text_response(resp: requests.Response, stub: str) -> None:
    ...
    if resp.status_code == 400:
        ...
        return
    raise AssertionError(f"unexpected status for data service {stub}: {resp.status_code} {resp.text}")


@pytest.mark.integration
def test_data_service_proxies() -> None:
    env = resolve_env()
    ...
    entries = services.get("data", [])
    assert entries, "no data services returned"
    for service in entries:
        stub = service.get("stub")
        assert stub, "missing stub for data service"
        logger.info("service:data enable %s", stub)
        ...
        proxy_url = f"{api_url}/services/data/{stub}?projectId={project_id}"
        logger.info("service:data proxy %s", stub)
        ...
        logger.info("service:data status %s -> %s", stub, proxy_resp.status_code)
