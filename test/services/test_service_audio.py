...
def assert_non_text_response(resp: requests.Response, stub: str) -> None:
    ...
    if resp.status_code == 400:
        ...
        return
    raise AssertionError(f"unexpected status for audio service {stub}: {resp.status_code} {resp.text}")


@pytest.mark.integration
def test_audio_service_proxies() -> None:
    env = resolve_env()
    ...
    entries = services.get("audio", [])
    assert entries, "no audio services returned"
    for service in entries:
        stub = service.get("stub")
        assert stub, "missing stub for audio service"
        logger.info("service:audio enable %s", stub)
        ...
        proxy_url = f"{api_url}/services/audio/{stub}?projectId={project_id}"
        logger.info("service:audio proxy %s", stub)
        ...
        logger.info("service:audio status %s -> %s", stub, proxy_resp.status_code)
