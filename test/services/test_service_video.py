...
def assert_non_text_response(resp: requests.Response, stub: str) -> None:
    ...
    if resp.status_code == 400:
        ...
        return
    raise AssertionError(f"unexpected status for video service {stub}: {resp.status_code} {resp.text}")


@pytest.mark.integration
def test_video_service_proxies() -> None:
    env = resolve_env()
    ...
    entries = services.get("video", [])
    assert entries, "no video services returned"
    for service in entries:
        stub = service.get("stub")
        assert stub, "missing stub for video service"
        logger.info("service:video enable %s", stub)
        ...
        proxy_url = f"{api_url}/services/video/{stub}?projectId={project_id}"
        logger.info("service:video proxy %s", stub)
        ...
        logger.info("service:video status %s -> %s", stub, proxy_resp.status_code)
